...
class NullSafeLLMEntityRelationExtractor(LLMEntityRelationExtractor):
    ...
    async def extract_for_chunk(
        ...
        prompt += (
            ...
            "\"value\": \"10^10 Па\"}}}.\n"
            "Фрагмент с маркером [Визуальный фрагмент] является описанием "
            "таблицы, графика, схемы или изображения. Извлекай из него обычные "
            "сущности предметной области, но не создавай сущность «картинка» или "
            "«таблица». Если указано, что значение визуально оценено, сохрани "
            "это как ограничение и не представляй значение как точное."
        )
        chunk_id = str(
